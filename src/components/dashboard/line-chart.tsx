...
const CustomTooltip = ({ active, payload, label }) => {
  if (active) {
    let { name, sales, conversions }: any = { ...payload[0].payload };
    return (
...
export const Line1 = () => {
  let colors = [
    { dataKey: 'sales', stroke: getColor('whiteGreen') },
    { dataKey: 'conversions', stroke: getColor('darkGreen') },
  ];
  const labels = [
    'Jan',
    'Feb',
  ...
  const data = Array.from(Array(12).keys()).map((i) => {
    return {
      name: labels[i],
      sales: random(100, 700),
      conversions: random(150, 700),
    };
  });
    ...
    <div style={{ width: '100%', height: 400 }}>
      <ResponsiveContainer>
        ...
        >
          ...
          <YAxis axisLine={true} tickLine={false} width={30} />
          <Tooltip
            content={
              <CustomTooltip
                active={undefined}
